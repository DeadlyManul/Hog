"""Графический интерфейс (GUI) для игры в в «Свинью».

В файле применяются возможности Python выходящие за рамки курса.
"""
...
class TextWidget(BetterWidget):
    """Класс TextWidget содержит изменяемую строку текста."""
...
class HogGUIException(BaseException):
    """HogGUI-специфическое исключение. Используется для выхода из игры."""
    pass

class HogGUI(Frame):
    """GUI на Tkinter для игры."""

    KILL = -9   # сигнал kill останавливает игру
    ...
    def __init__(self, parent, computer=False):
        ...
        parent   -- родительский виджет (должен быть root'ом)
    ...
    def init_restart(self):
        ...
        self.restart_button = Button(self, text='Начать сначала',
    ...
    def strategy(self, score, opp_score):
        ...
        self.clear_dice()
        self.dice_count = 0
        self.status_label.text = '{} использовал бросков: {} .'.format(name(self.who),
                                                               result)
        self.switch()
        return result
