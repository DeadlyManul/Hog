...
def test_all(project_name, tests=TESTS):
    ...
    for test in tests:
        underline('Тест {0}'.format(test.name))
        try:
...
def check_func(func, tests,
    ...
    for input, output, *desc in tests:
        try:
            ...
            fail_msg = "Функция {0} упала".format(func.__name__)
            if desc:
                print(fail_msg, desc[0])
            else:
                print(fail_msg, "аргументы", in_print(input))
            traceback.print_exception(*sys.exc_info(), limit=2)
            code += 1
            continue
        if not comp(val, output):
            wrong_msg = "Неверный результат для {0}:".format(func.__name__)
            if desc:
                print(wrong_msg, desc[0])
            else:
                print(wrong_msg, "аргументы", in_print(input))
                print('\tполучен результат {0}, ожидался результат {1}\n'.format(val, out_print(output)))
            code += 1
    return code
...
def check_for_updates(index, filenames, version):
    print('Ты запустил версию', version, 'autograder.py')
    try:
        remotes = {}
        ...
        print('.'*20) #print("Couldn't check remote autograder")
        return
...
def run_tests(name, remote_index, autograder_files, version, **kwargs):
    ...
    parser.add_argument('-q', '--question', type=int,
                        help='Запускает тесты отдельной задачи')
    parser.add_argument('-v', '--version', action='store_true',
                        help='Печатает верисию autograder.py и завершается')
