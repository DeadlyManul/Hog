"""Скрипт автоматического оценивания проекта «Игра в «Свинью».

Требует следующие файлы в текущем директории:
...
Этот файл включает возможности Python нерасссматриваемые в курсе.
"""
...
try:
    import hog      # Код студента
except (SyntaxError, IndentationError) as e:
    import traceback
    print('Твоя программа содержит синтаксическую ошибку:')
...
def problem4(grades):
  """Тестирует is_prime."""
  return check_doctest('is_prime', hog)

@test
def problem5(grades):
    """Тестирует play используя fixed dice."""
    always = hog.always_roll
    hog.four_sided = make_test_dice(1)
    hog.six_sided = make_test_dice(3)

    test_suite = [((always(5),  always(5)), (106, 46)),
                  ((always(2),  always(2)), (57, 104)),
                  ((always(2), always(10)), (7, 126)),
                  ((always(0),  always(0)), (74, 106)), # всегда 0
                  ((always(0),  always(2)), (108, 94))]
...
@test
def problem9(grades):
    """Тестирует prime_strategy."""
    if check_doctest('prime_strategy', hog):
        return True
    test_suite = [((12, 10, 5, 4), 4),    # не простое
                  ((32, 34, 5, 4), 0),    # простое, довесок себе 
                  ((22, 34, 5, 4), 4),    # простое, довесок противнику
                  ((31, 12, 5, 4), 4)]    # простое, равны -- нереальзуемо
    failed = check_func(hog.prime_strategy, test_suite)
    return failed


# @test
# def problem9(grades):
#     """Тестирует final_strategy."""
...
project_info = {
    'name': 'Project 1: Hog',
    'remote_index': 'http://yandex.ru',#'http://inst.eecs.berkeley.edu/~cs61a/fa13/proj/hog/',
    'autograder_files': [
        'hog_grader.py',
        'autograder.py',
    ],
